    ...
    <Card className="shadow-[var(--shadow-card)] border-border">
      <CardContent className="p-5">
        <div className="flex items-start justify-between">
          ...
          <div
            className={`grid h-10 w-10 place-items-center rounded-lg ${accent ?? "bg-accent text-primary"}`}
          >
...
function Dashboard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const totalDue = (invoices.data ?? []).reduce(
        (s, i) => s + Number(i.amount) - Number(i.amount_paid),
        0,
      );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Vue d'ensemble de votre établissement en temps réel."
      />
        ...
        {isLoading ? (
          ...
          <>
            ...
            <StatCard
              icon={Wallet}
              label="Encaissements"
              value={`${data!.totalPaid.toLocaleString("fr-FR")} F`}
              hint={`Reste dû: ${data!.totalDue.toLocaleString("fr-FR")} F`}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2 shadow-[var(--shadow-card)]">
          ...
          <CardContent className="text-sm text-muted-foreground">
            ...
            <ol className="mt-4 space-y-2 text-foreground list-decimal pl-5">
              <li>
                Créez votre école dans <strong>Mon école</strong>.
              </li>
        ...
        <Card className="shadow-[var(--shadow-card)]">
          <CardHeader>
            <CardTitle className="font-display text-lg flex items-center gap-2">
              <AlertCircle className="h-5 w-5 text-warning" />À surveiller
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {data ? `${data.totalDue.toLocaleString("fr-FR")} F` : "—"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Taux de présence</span>
              <span className="font-semibold">{data ? `${data.attendanceRate}%` : "—"}</span>
            </div>
          </CardContent>
        </Card>
      </div>
