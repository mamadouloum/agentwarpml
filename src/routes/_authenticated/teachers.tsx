...
function TeachersPage() {
  ...
  assignments.forEach((a: any) => {
    (byTeacher[a.teacher_user_id] ||= []).push(a);
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat
          ...
          hint={`${teacherProfiles.length - assignedTeachers} sans classe`}
        />
        <Stat
          icon={<BookOpen className="h-4 w-4" />}
          label="Classes couvertes"
          value={`${coveredCount}/${classes.length}`}
        />
        <Stat
                  ...
                  <CardContent className="p-0">
                    <div className="px-4 py-3 border-b border-border flex items-center justify-between gap-3 flex-wrap">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => autoAssignAllClasses(p.id)}
                      >
                        ...
                        <TableBody>
                          {items.map((a: any) => (
                            <TableRow key={a.id}>
                              ...
                              <TableCell>
                                {a.subjects?.name ? (
                                  <Badge variant="outline">{a.subjects.name}</Badge>
                                ) : (
            ...
            classes.map((c: any) => (
              <ClassEditor
                ...
                onAdd={async (teacherId, subjectId) => {
                  if (!schoolId) return;
                  const dup = (byClass[c.id] ?? []).some(
                    (a: any) =>
                      a.teacher_user_id === teacherId &&
                      (a.subject_id ?? null) === (subjectId || null),
                  );
