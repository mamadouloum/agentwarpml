          ...
          <div className="flex justify-end">
            <Dialog open={openRoute} onOpenChange={setOpenRoute}>
              <DialogTrigger asChild>
                <Button><Plus className="h-4 w-4 mr-2" />Nouvelle ligne</Button>
              </DialogTrigger>
              <DialogContent>
                ...
                <form onSubmit={createRoute} className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><Label>Chauffeur</Label><Input name="driver_name" /></div>
                    <div><Label>Téléphone</Label><Input name="driver_phone" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><Label>Immatriculation</Label><Input name="vehicle_plate" /></div>
                    <div><Label>Capacité</Label><Input type="number" name="capacity" /></div>
                  </div>
                  <div><Label>Tarif mensuel</Label><Input type="number" step="0.01" name="monthly_fee" /></div>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Nom</TableHead><TableHead>Chauffeur</TableHead>
                    <TableHead>Véhicule</TableHead><TableHead>Capacité</TableHead>
                    <TableHead>Tarif</TableHead><TableHead></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {routes.map((r: any) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell>{r.driver_name} {r.driver_phone && `· ${r.driver_phone}`}</TableCell>
                      <TableCell>{r.vehicle_plate}</TableCell>
                      <TableCell>{r.capacity}</TableCell>
                      <TableCell>{Number(r.monthly_fee).toLocaleString()} F</TableCell>
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" onClick={() => removeRoute(r.id)}><Trash2 className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  ))}
                  {routes.length === 0 && (
                    <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">Aucune ligne</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </CardContent>
        ...
        <TabsContent value="subs" className="space-y-4">
          <div className="flex justify-end">
            <Dialog open={openSub} onOpenChange={setOpenSub}>
              ...
              <DialogContent>
                <DialogHeader><DialogTitle>Abonnement transport</DialogTitle></DialogHeader>
                <form onSubmit={createSub} className="space-y-3">
                  ...
                  <div><Label>Point d'embarquement</Label><Input name="pickup_point" /></div>
                  <DialogFooter>
                    <Button type="submit" disabled={saving}>
                      {saving && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Enregistrer
                    </Button>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>Élève</TableHead><TableHead>Ligne</TableHead>
                    <TableHead>Point</TableHead><TableHead>Montant</TableHead>
                    <TableHead>Début</TableHead><TableHead></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {subs.map((s: any) => (
                    <TableRow key={s.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" onClick={() => removeSub(s.id)}><Trash2 className="h-4 w-4" /></Button>
                      </TableCell>
                    </TableRow>
                  ))}
                  {subs.length === 0 && (
