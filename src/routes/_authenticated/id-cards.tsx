...
function IdCardsPage() {
  ...
  const { data: school } = useQuery({
    ...
    enabled: !!schoolId,
    queryFn: async () =>
      (await supabase.from("schools").select("name,address").eq("id", schoolId!).maybeSingle())
        .data,
  });
  ...
  const { data: students = [] } = useQuery({
    ...
    queryFn: async () => {
      let req = supabase
        .from("students")
        .select("id,first_name,last_name,matricule,birth_date,photo_url,classes(name)")
        .order("last_name");
  ...
  async function importCsv(file: File) {
    try {
      ...
      if (!matched.length) {
        toast.error("Aucun matricule reconnu dans le fichier.");
        return;
      }
  ...
  async function drawCard(
    ...
    if (logoData) {
      try {
        doc.addImage(logoData, "PNG", x + 2, y + 1.5, 9, 9);
      } catch {
        /* ignore */
      }
    }
    ...
    if (photoData) {
      try {
        doc.addImage(photoData, "JPEG", phX, phY, phW, phH);
      } catch {
        /* ignore */
      }
    } else {
    ...
    if (opts.showBirth && st.birth_date)
      line("Né(e) le", new Date(st.birth_date).toLocaleDateString("fr-FR"));
    ...
    const foot =
      opts.footerText || (opts.showMotto ? (branding?.motto ?? school?.address ?? "") : "");
    ...
    try {
      ...
      if (batchFormat === "pdf") {
        ...
        for (let i = 0; i < targets.length; i++) {
          ...
          const safe = `${st.matricule || st.id}_${st.last_name}_${st.first_name}`.replace(
            /[^a-z0-9_-]/gi,
            "_",
          );
        ...
        a.href = url;
        a.download = `cartes_scolaires_${opts.year}.zip`;
        a.click();
    ...
    <div className="space-y-6">
      ...
      <Tabs value={mode} onValueChange={(v) => setMode(v as any)}>
        ...
        <Card className="shadow-[var(--shadow-card)] border-border mt-4">
          <CardHeader>
            <CardTitle className="font-display text-base">Modèle</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                type="color"
                value={opts.primaryHex}
                onChange={(e) => setOpts({ ...opts, primaryHex: e.target.value })}
                className="h-10 p-1"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                value={opts.year}
                onChange={(e) => setOpts({ ...opts, year: e.target.value })}
              />
            </div>
            <div className="space-y-1.5 col-span-2">
              ...
              <Input
                ...
                placeholder={branding?.motto ?? "Devise / adresse"}
              />
            </div>
            <div className="flex items-center justify-between gap-2">
              <Label>Matricule</Label>
              <Switch
                checked={opts.showMatricule}
                onCheckedChange={(v) => setOpts({ ...opts, showMatricule: v })}
              />
            </div>
            <div className="flex items-center justify-between gap-2">
              <Label>Classe</Label>
              <Switch
                checked={opts.showClass}
                onCheckedChange={(v) => setOpts({ ...opts, showClass: v })}
              />
            </div>
            <div className="flex items-center justify-between gap-2">
              <Label>Date naiss.</Label>
              <Switch
                checked={opts.showBirth}
                onCheckedChange={(v) => setOpts({ ...opts, showBirth: v })}
              />
            </div>
            <div className="flex items-center justify-between gap-2">
              <Label>Devise</Label>
              <Switch
                checked={opts.showMotto}
                onCheckedChange={(v) => setOpts({ ...opts, showMotto: v })}
              />
            </div>
          </CardContent>
        </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                  ...
                  >
                    <div
                      ...
                      style={{ background: opts.primaryHex }}
                    >
                      {branding?.logo_url ? (
                        <img
                    ...
                    <div className="flex gap-3 p-3">
                      <div className="w-20 h-28 rounded bg-slate-100 border grid place-items-center overflow-hidden shrink-0">
                        {singlePhotoUrl ? (
                          <img src={singlePhotoUrl} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-slate-400 font-bold text-xl">
                            {(single.first_name?.[0] ?? "") + (single.last_name?.[0] ?? "")}
                          </span>
                        )}
                      </div>
                      <div className="text-xs space-y-1 min-w-0">
                        <div className="font-bold text-sm truncate">
                          {single.last_name} {single.first_name}
                        </div>
                        {opts.showMatricule && (
                          <div>
                            <span className="text-slate-500">Matricule </span>
                            <b>{single.matricule}</b>
                          </div>
                        )}
                        {opts.showClass && (
                          <div>
                            <span className="text-slate-500">Classe </span>
                            <b>{single.classes?.name ?? "—"}</b>
                          </div>
                        )}
                        {opts.showBirth && single.birth_date && (
                          <div>
                            <span className="text-slate-500">Né(e) </span>
                            <b>{new Date(single.birth_date).toLocaleDateString("fr-FR")}</b>
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="px-3 pb-2 text-[10px] italic text-slate-500 flex justify-between border-t pt-1 mx-3">
                      <span className="truncate">
                        {opts.footerText ||
                          (opts.showMotto ? (branding?.motto ?? school?.address ?? "") : "")}
                      </span>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Button variant="outline" onClick={() => uploadRef.current?.click()}>
                        ...
                        {single.photo_url ? "Remplacer la photo" : "Téléverser une photo"}
                      </Button>
                    </div>
            ...
            <CardContent className="space-y-4 p-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  >
                    {batchFiltered.length > 0 && batchFiltered.every((s: any) => selected.has(s.id))
                      ? "Tout désélectionner"
                      : "Tout sélectionner"}
                  </Button>
