  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Disposition des élèves dans la salle (glisser-déposer)"
      />
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            <form onSubmit={onCreate} className="space-y-4">
              <DialogHeader>
                <DialogTitle>Créer un plan de classe</DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-2">
                <Label>Nom</Label>
                <Input name="name" required defaultValue="Plan principal" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  <Label>Rangées</Label>
                  <Input name="rows" type="number" min={1} max={20} defaultValue={5} />
                </div>
                <div className="space-y-2">
                  <Label>Colonnes</Label>
                  <Input name="cols" type="number" min={1} max={20} defaultValue={6} />
                </div>
              </div>
              <DialogFooter>
...
function PlanEditor({ plan }: { plan: any }) {
  ...
  const assignedIds = useMemo(
    () => new Set(assignments.map((a: any) => a.student_id)),
    [assignments],
  );
  ...
  async function placeStudent(studentId: string, row: number, col: number) {
    ...
    const { error } = await (supabase as any)
      .from("seating_assignments")
      .insert({ plan_id: plan.id, student_id: studentId, row, col });
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              Élèves ({unassigned.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 max-h-[600px] overflow-y-auto">
            {unassigned.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-4">Tous placés</div>
            ) : (
